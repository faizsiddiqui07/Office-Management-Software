...
/**
 * The sign-in screen. `adding` = reached from "Add another account" in the user menu
 * (/login/add): someone is already signed in and wants a second account on this device
 * (lib/accounts.js). The form works as usual; what changes is that we DON'T bounce them to
 * the dashboard for being signed in, and after signing in we hard-reload so nothing of the
 * first account's cache survives. A separate route rather than ?add=1 because a static
 * page can't read search params without a Suspense boundary.
 */
export function LoginScreen({ adding = false }) {
  ...
  React.useEffect(() => {
    if (!adding && !isLoading && user) router.replace('/dashboard');
  }, [adding, isLoading, user, router]);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const isNew = adding && signedIn?.id && !known.has(String(signedIn.id));
      toast.success(adding ? (isNew ? 'Account added' : 'Signed in again') : 'Welcome back!');
      if (adding) window.location.href = '/dashboard';
          ...
          <div className="w-full max-w-md">
            ...
            <div className="space-y-2">
              <h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">
                {adding ? 'Add another account' : 'Welcome back'}
              </h1>
              <p className="text-sm text-muted-foreground">
                {adding && user
                  ? `Sign in to a second account. ${user.name} stays signed in — switch between them from your profile menu.`
                  : 'Sign in to your office workspace to continue.'}
              </p>
            </div>
            ...
            {adding && user ? (
              ...
              >
                <ArrowLeft className="size-4" /> Back to {user.name?.split(' ')[0] || 'the app'}
              </button>
            ) : null}
